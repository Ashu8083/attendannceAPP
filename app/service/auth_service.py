...
class AuthService:
    ...
    def verify_otp(self,otp_schema : OTPSchema) :
        ...
        if otp.otp != otp_schema.otp :
            raise Exception(f"otp not found or invalid")
        access_token = create_access_token(user.id,user_role= user.role)
        refresh_token = create_refresh_token(user.id,user_role= user.role)
        return {
