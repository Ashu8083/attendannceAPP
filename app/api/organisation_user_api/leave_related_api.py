...
@leave_request_router.post("/request-leave",response_model=LeaveResponse)
# dependencies=[Depends(PermissionChecker("leave.request.view","ORGANISATION"))])
def create_leave_request(request : Request,
                        leave_request_schema : LeaveCreate,
                        credentials = Security(bearer_scheme)
                      , leave_service: LeaveService = Depends(get_leave_service))  :

    return  leave_service.apply_leave(leave_request_schema,employee_id = request.state.auth.employee_id)
