...
from app.schemas.department_schema import  DepartmentCreate,ListOfDepartment,DepartmentResponse
...
@department_router.get("/get-department/{department_name}",response_model=DepartmentResponse, dependencies=[Depends(PermissionChecker("department","ORGANISATION"))])
...
@department_router.post("/create-department}",response_model=DepartmentResponse,dependencies=[Depends(PermissionChecker("department","ORGANISATION"))])
...
@department_router.put("/department",response_model=DepartmentResponse,dependencies=[Depends(PermissionChecker("department","ORGANISATION"))])
def soft_delete_department(request:Request ,department_name : str  , department_service = Depends(get_department_service)):
    return department_service.soft_delete_department(department_name,oranisation_id=request.state.auth.oranisation_id)
@department_router.get("/get-all-department",response_model= ListOfDepartment,dependencies=[Depends(PermissionChecker("department","ORGANISATION"))])
