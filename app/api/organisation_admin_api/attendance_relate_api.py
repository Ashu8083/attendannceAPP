...
@attendance_manager.patch(
    "/update",
    response_model=AttendanceResponse,
    dependencies=[Depends(PermissionChecker("attendance.update", "ORGANISATION"))],
)
def update_attendance(
    request: Request,
    attendance_update_schema: AttendanceUpdate,
    attendance_service: AttendanceService = Depends(get_attendance_service),
):
    attendance_update = attendance_service.update_employee_attendance(
        organisation_id=request.state.auth.organisation_id,
        attendance_update_schema=attendance_update_schema,
    )
    if attendance_update:
        logger.info(
            f"attendance updated by {request.state.auth.user_id} from {request.state.auth.organisation_id} organisation for {attendance_update_schema.dict()}"
        )

    return attendance_update


@attendance_manager.get(
    "/abesent-list",
    dependencies=[Depends(PermissionChecker("attendance.view", "ORGANISATION"))],
)
def absent_list(
    request: Request,
...
def mark_absent(
    ...
    return attendance_service.marked_absent_employee(
        organisation_id=request.state.auth.organisation_id,
